...
class Problem14 : DailyProblem<Int> {
...
//            val yMin = ys.min()
                ...
                paths.forEach { p ->
                    p.path.zipWithNext().forEach { line ->
                        toIntRange(line.first.y, line.second.y).forEach { y ->
                            toIntRange(line.first.x, line.second.x).forEach { x ->
                                grid[y][x] = '#'
                            }
                        }
                    }
    ...
    fun solvePart0(): Int {
        val cave = Cave.fromPaths(toPaths(data0))
//        println(cave)
        return generateSequence { cave.dripSand() }.count()
    }

    override fun solvePart1(): Int {
        val cave = Cave.fromPaths(toPaths(data1))
        return generateSequence { cave.dripSand() }.count()
    }
    ...
    fun toPaths(data: List<String>): List<Path> {
        return data.map { line ->
            line.split(" -> ").map { point ->
                point.split(",").let { Pt(it[0].toInt(), it[1].toInt()) }
            }.let { points ->
                Path(points)
            }
        }
