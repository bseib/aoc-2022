class Problem7 : DailyProblem<Int> {
    ...
    data class DirNode(val name: String, val contents: MutableList<Node>) : Node() {
        ...
        fun size(): Int = contents.sumOf { node ->
            when (node) {
                is DirNode -> node.size()
        ...
        fun getDirAtRelativePath(path: List<String>): DirNode {
            ...
            else contents.filterIsInstance<DirNode>().find { it.name == path[0] }
                ?.getDirAtRelativePath(path.subList(1, path.size))
    ...
    fun solvePart0(): Int {
        val root = parseCommandsIntoFilesystem(makeRootNode(), data0)
...
//                }
                ...
                currentDir.addNode(parseContentAsNode(it))
            }
        }
        return root
    }

    fun parseContentAsNode(line: String): Node {
        return if (line.startsWith("dir ")) {
            DirNode(line.substring(4), mutableListOf())
        }
        else {
            val index = line.indexOfFirst { it == ' ' }
            FileNode(line.substring(index + 1), line.substring(0, index).toInt())
        }
    }

    fun updateCWD(cwd: MutableList<String>, cwdCommand: String) {
        val cdArg = cwdCommand.substring(5)
        when (cdArg) {
            "/" -> cwd.clear()
            ".." -> cwd.removeAt(cwd.size - 1)
            else -> cwd.add(cdArg)
        }
    }
